...
<div class="max-w-md w-full mx-auto bg-white border border-coffee-sand rounded-3xl p-7 sm:p-8 shadow-sm space-y-6 my-12 animate-fade-in">
    <div class="flex items-start justify-between gap-4">
        <div class="space-y-2">
            <p class="text-[10px] text-coffee-milk uppercase font-bold tracking-wide font-mono">Staff authentication</p>
            <h2 class="text-3xl font-serif font-bold text-coffee-dark">Đăng nhập nội bộ</h2>
            <p class="text-sm text-coffee-milk">Tài khoản nhân viên, quản lý hoặc quầy pha chế.</p>
        </div>
        <div class="demo-help">
    ...
    <form onsubmit="return handleMockLogin(event)" class="space-y-4">
        <div class="space-y-1">
            <label class="text-[10px] font-mono tracking-wider font-bold uppercase text-coffee-milk block">Tên tài khoản</label>
            <input type="text" id="username" autocomplete="username" placeholder="Nhập username" required class="w-full text-sm px-4 py-3 bg-coffee-light border border-coffee-sand rounded-xl focus:outline-none focus:border-coffee-rust focus:bg-white transition-all font-mono">
        </div>
        
        <div class="space-y-1">
            <label class="text-[10px] font-mono tracking-wider font-bold uppercase text-coffee-milk block">Mật khẩu</label>
            <input type="password" id="password" autocomplete="current-password" required class="w-full text-sm px-4 py-3 bg-coffee-light border border-coffee-sand rounded-xl focus:outline-none focus:border-coffee-rust focus:bg-white transition-all font-mono">
        </div>
    ...
    <div class="border-t border-coffee-sand/50 pt-4 text-center space-y-2">
        <div class="grid grid-cols-2 gap-2">
            <a href="pin-login.jsp" class="border border-coffee-sand rounded-xl py-2.5 px-3 hover:border-coffee-rust text-xs text-coffee-dark bg-coffee-light font-bold hover:bg-white transition-colors">
                Nhập PIN
            </a>
...
<script>
    async function handleMockLogin(e) {
        ...
        try {
            ...
            if (!res.ok) {
                alert(data.error || 'Đăng nhập thất bại. Vui lòng kiểm tra lại tài khoản.');
                return false;
            }
