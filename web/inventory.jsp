...
<head>
    ...
    <link
        href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&family=Playfair+Display:ital,wght@0,600;0,700;1,600&family=JetBrains+Mono:wght@400;500;600&display=swap"
        rel="stylesheet">
    <style>
        body {
...
<body class="min-h-screen flex flex-col dot-grid-bg relative selection:bg-coffee-rust/20 selection:text-coffee-rust">
    ...
    <!-- TOP NAVIGATION BAR -->
    <nav
        class="border-b border-coffee-sand/70 bg-coffee-bg/90 backdrop-blur sticky top-0 z-40 px-6 py-4 transition-all">
        <div class="max-w-7xl mx-auto flex items-center justify-between">
            ...
            <div class="flex items-center gap-3">
                <div id="connection-status">
                    <div
                        class="bg-amber-50 text-amber-800 border border-amber-200/50 px-3 py-1 rounded-full text-xs flex items-center gap-1.5 font-medium">
                ...
                <div
                    class="hidden md:flex bg-coffee-light border border-coffee-sand/60 px-3 py-1 rounded-full items-center gap-1.5 font-mono text-xs text-coffee-dark font-medium">
    ...
    <div id="flash-banner-container" class="hidden fixed bottom-6 right-6 z-50 max-w-sm w-full animate-bounce">
        <div id="flash-banner"
            class="bg-coffee-dark text-white border border-coffee-rust/50 px-4 py-3 rounded-2xl flex items-center gap-2.5 shadow-xl">
    ...
    <main class="flex-1 max-w-7xl w-full mx-auto p-4 sm:p-6 lg:p-8 flex flex-col justify-start">
        ...
        <div class="space-y-6">
            ...
            <!-- Header section -->
            <div
                class="bg-white border border-coffee-sand/70 p-5 rounded-3xl shadow-xs flex justify-between items-center">
                <div>
                    <h2 class="text-xl font-serif italic font-bold text-coffee-dark flex items-center gap-2">
                        <span>📦</span> Kho Nguyên Liệu & Công Thức
                    </h2>
                    <p class="text-xs text-coffee-milk font-medium">Theo dõi lượng tồn kho thực tế, nhập kho thanh toán
                        nguyên liệu và giám sát công thức pha chế đồ uống.</p>
                </div>
                <div
                    class="bg-[#FAF7EE] border border-coffee-sand px-3.5 py-1.5 rounded-full text-xs font-bold font-mono text-coffee-rust flex items-center gap-1">
                    <span>Trạng thái: </span> <span id="inventory-low-alert-dot"
                        class="w-2.5 h-2.5 bg-emerald-500 rounded-full inline-block"></span>
            ...
            <div class="flex border-b border-coffee-sand gap-1">
                <button onclick="switchTab('tab-stock')" id="btn-tab-stock"
                    class="px-5 py-2.5 font-bold text-xs rounded-t-2xl transition-all border-t border-x border-transparent bg-transparent text-coffee-milk hover:text-coffee-rust">
                    📦 Tồn Kho Hiện Tại
                </button>
                <button onclick="switchTab('tab-import')" id="btn-tab-import"
                    class="px-5 py-2.5 font-bold text-xs rounded-t-2xl transition-all border-t border-x border-transparent bg-transparent text-coffee-milk hover:text-coffee-rust flex items-center gap-1.5">
                    💳 Nhập Kho & Thanh Toán
                </button>
                <button onclick="switchTab('tab-recipes')" id="btn-tab-recipes"
                    class="px-5 py-2.5 font-bold text-xs rounded-t-2xl transition-all border-t border-x border-transparent bg-transparent text-coffee-milk hover:text-coffee-rust">
                    📋 Công Thức & Khả Dụng Món
                </button>
                <button onclick="switchTab('tab-logs')" id="btn-tab-logs"
                    class="px-5 py-2.5 font-bold text-xs rounded-t-2xl transition-all border-t border-x border-transparent bg-transparent text-coffee-milk hover:text-coffee-rust">
                    🧾 Nhật Ký Nhập Kho
                </button>
            </div>
            ...
            <div id="tab-stock" class="space-y-4">
                <div class="bg-white border border-coffee-sand rounded-3xl p-6 shadow-xs space-y-4">
                    <div class="flex justify-between items-center border-b border-coffee-light pb-3">
                        <div>
                            <h3 class="font-serif italic font-bold text-base text-coffee-dark">Trạng thái tồn kho cà phê
                                & phụ liệu</h3>
                            <p class="text-[10px] text-coffee-milk">Các mức định lượng trong kho để kịp thời cảnh báo
                                pha chế</p>
                        </div>
                        <button onclick="syncAllState()"
                            class="text-xs font-mono font-bold text-coffee-rust hover:underline">
                            🔄 Cập nhật tức thời
                        </button>
                    </div>
            ...
            <div id="tab-import" class="hidden grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
                ...
                <div class="lg:col-span-2 bg-white border border-coffee-sand rounded-3xl p-6 shadow-xs space-y-4">
                    <div class="border-b border-coffee-light pb-2">
                        <h3 class="font-serif italic font-bold text-base text-coffee-dark">Phiếu đề xuất nhập hàng
                            nguyên liệu</h3>
                        <p class="text-[10px] text-coffee-milk">Vui lòng điền định lượng (số lượng) mong muốn nhập thêm
                            cho từng loại nguyên liệu.</p>
                    </div>
                ...
                <div
                    class="bg-white border border-coffee-rust/35 rounded-3xl p-6 shadow-md space-y-4 relative overflow-hidden">
                    <div class="absolute -top-12 -right-12 w-28 h-28 bg-coffee-rust/10 rounded-full blur-xl"></div>

                    <h3
                        class="font-serif font-bold text-coffee-dark text-sm border-b border-coffee-sand pb-2 flex items-center gap-1.5">
                        <span>🧾</span> CHI TIẾT THANH TOÁN MUA HÀNG
                    </h3>

                    <div id="checkout-receipt-lines"
                        class="space-y-2 max-h-[220px] overflow-y-auto text-xs pr-1 font-mono text-coffee-milk">
                        <!-- Dynamic receipt -->
                    </div>
                    ...
                    <div class="border-t border-dashed border-coffee-sand pt-3 space-y-1">
                        ...
                        <div class="flex justify-between text-sm text-coffee-dark pt-1">
                            ...
                            <span id="checkout-receipt-total" class="font-mono font-bold text-coffee-rust text-base">0
                                ₫</span>
                        </div>
                    </div>

                    <div class="space-y-2">
                        <button onclick="submitImportRequest()" id="btn-submit-import"
                            class="w-full bg-coffee-rust hover:bg-coffee-rust/85 text-white text-xs font-bold py-3 px-4 rounded-xl shadow-xs cursor-pointer select-none transition-all duration-150 flex items-center justify-center gap-1.5 focus:outline-none">
                            <span>💳 Thanh toán & Nhập kho quốc gia</span>
                        </button>
                        <p class="text-[9px] text-coffee-milk text-center italic leading-tight">Thanh toán sẽ ngay lập
                            tức được giải ngân từ tài khoản quỹ của quán, đồng thời cộng thẳng số lượng vào tồn kho thực
                            tiễn.</p>
                    </div>
                </div>
            ...
            <div id="tab-recipes" class="hidden space-y-4">
                <div class="bg-white border border-coffee-sand rounded-3xl p-6 shadow-xs space-y-4">
                    <div class="border-b border-coffee-light pb-2">
                        <h3 class="font-serif italic font-bold text-base text-coffee-dark">Đồ uống, Bánh ngọt & Định
                            lượng cơ bản</h3>
                        <p class="text-[10px] text-coffee-milk">Nhấp xem cấu trúc nguyên liệu cơ bản cho 1 phần bán
                            chuẩn.</p>
                    </div>
            ...
            <div id="tab-logs" class="hidden space-y-4">
                <div class="bg-white border border-coffee-sand rounded-3xl p-6 shadow-xs space-y-4">
                    <div class="flex justify-between items-center border-b border-coffee-light pb-3">
                        <div>
                            <h3 class="font-serif italic font-bold text-base text-coffee-dark">Nhật ký chi phí nhập kho
                                & thanh toán</h3>
                            <p class="text-[10px] text-coffee-milk font-normal">Tổng hợp các đợt phát sinh hoá đơn mua
                                hàng phụ liệu</p>
                        </div>
                        <span
                            class="text-xs bg-coffee-light text-coffee-rust font-bold px-3 py-1 rounded-xl border border-coffee-sand/70 font-mono"
                            id="logs-expenses-total">Tổng chi: 0 ₫</span>
                    </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left text-xs border-collapse">
                            <thead>
                                <tr
                                    class="border-b border-coffee-sand text-coffee-milk uppercase tracking-wider text-[10px] font-mono">
    ...
    <script>
        ...
        // Recipes are now loaded dynamically from the API
        ...
        function renderRecipes() {
            ...
            menu.forEach(item => {
                ...
                // Construct the client recipe from the loaded inventory and the item's recipes
                const clientRecipe = [];
                if (item.recipes && item.recipes.length > 0) {
                    item.recipes.forEach(r => {
                        const ing = inventory.find(i => i.id === r.ingredientId);
                        if (ing) {
                            clientRecipe.push({ name: ing.name, qty: r.quantity + " " + ing.unit });
                        }
                    });
                }
                    ...
                    <div class="border ${isAvailable ? 'border-coffee-sand/70 bg-coffee-light/35' : 'border-red-200 bg-red-50/5'} p-4.5 rounded-3xl space-y-3">
                        <div class="flex justify-between items-start">
                            <div>
                                ...
                                <h4 class="font-serif font-bold text-sm text-coffee-dark mt-1">${item.nameVi || item.name}</h4>
