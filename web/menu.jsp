...
<script>
    ...
    function drawCustMenuList() {
        ...
        list.forEach(item => {
            ...
            container.innerHTML += `
                <div onclick="triggerCustomerSettings('${item.id}')" class="bg-white hover:bg-coffee-light/40 border border-coffee-sand/70 hover:border-coffee-rust/50 transition-all rounded-3xl p-4 flex flex-col justify-between cursor-pointer group shadow-xs">
                    <div class="space-y-3">
                        ${item.image ? `
                        <div class="w-full aspect-[4/3] rounded-2xl overflow-hidden bg-coffee-light border border-coffee-sand/30 relative">
                            <img src="${item.image}" alt="${item.name}" referrerpolicy="no-referrer" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300">
                        </div>
                        ` : ''}
                        <div class="space-y-1.5">
                            <div class="flex items-center justify-between gap-2">
                                ...
                                <span class="font-mono font-bold text-coffee-rust text-[13px]">
                                    ${formatVND(item.price)}
                                </span>
                            </div>
                            <h4 class="font-serif font-bold text-coffee-dark text-sm group-hover:text-coffee-rust transition-colors leading-tight">
                                ${item.name}
                            </h4>
                    ...
                    <div class="flex items-center justify-between pt-3 mt-3 border-t border-coffee-sand/25">
                        ...
                        <div class="w-6 h-6 rounded-full bg-coffee-light text-coffee-rust flex items-center justify-center text-[10px] group-hover:bg-coffee-rust group-hover:text-white transition-all duration-200">
                            ＋
                        </div>
                    </div>
                </div>
    ...
    function drawCustCartList() {
        ...
        total.innerText = formatVND(totalValue);
    }
