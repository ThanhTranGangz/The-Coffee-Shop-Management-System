                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div class="space-y-1">
                        ...
                        <select id="staff-role" class="w-full text-xs px-3 py-2 bg-coffee-light border border-coffee-sand rounded-xl text-coffee-dark focus:outline-none focus:border-coffee-rust cursor-pointer font-bold">
                            ...
                            <option value="manager">Giám sát (Manager)</option>
                        </select>
                    </div>
                </div>
...
<script>
    ...
    // Seeding dynamic storage for staff accounts
    let staffRoster = JSON.parse(localStorage.getItem('staff_roster'));
    if (!staffRoster || staffRoster.length === 0 || !staffRoster.some(s => s.username === 'waiter1')) {
        staffRoster = [
    ...
    let crmCustomers = JSON.parse(localStorage.getItem('local_member_db'));
    if (!crmCustomers || crmCustomers.length === 0) {
        crmCustomers = [
            { phone: '0909123456', name: 'Trần Thị Thuỷ Tiên', rank: 'Platinum', points: 740, email: 'thuytien@gmail.com', pref: 'Espresso', discount: 'Giảm 15% tổng hoá đơn' },
            { phone: '0901234567', name: 'Lê Hoàng Phong', rank: 'Gold', points: 320, email: 'hoangphong@gmail.com', pref: 'Tea', discount: 'Giảm 10% tổng hoá đơn' },
            { phone: '0987654321', name: 'Nguyễn Minh Quân', rank: 'Silver', points: 120, email: 'minhquan@gmail.com', pref: 'Special', discount: 'Giảm 5% tổng hoá đơn' }
        ];
        ...
        list.forEach(s => {
            ...
            tbody.innerHTML += `
                <tr class="hover:bg-coffee-light/25 transition-colors">
                    <td class="py-3 px-4">
                        <div class="font-bold text-coffee-dark">${s.name}</div>
                    ...
                    <td class="py-3 px-4 text-coffee-milk font-mono text-[11px]">${s.shift}</td>
                    <td class="py-3 px-4">
                        <button onclick="toggleRosterStat(${s.id})" class="text-[10px] font-bold px-2.5 py-0.5 rounded-full cursor-pointer ${s.active ? 'bg-emerald-100 text-emerald-800 border border-emerald-250' : 'bg-red-100 text-red-800 border border-red-200'}">
                            ${s.active ? '● Đang làm trực' : '○ Nghỉ ca'}
                        </button>
                    </td>
    ...
    function toggleRosterStat(id) {
        const item = staffRoster.find(s => s.id === id);
        if (item) {
            item.active = !item.active;
            localStorage.setItem('staff_roster', JSON.stringify(staffRoster));
            drawStaffRoster();
            flashNotify(`🧑‍🤝‍🧑 Đã đổi trạng thái ca của "${item.name}"!`);
        }
    }
    ...
    function deleteRosterItem(id) {
        ...
        staffRoster = staffRoster.filter(s => s.id !== id);
        localStorage.setItem('staff_roster', JSON.stringify(staffRoster));
        drawStaffRoster();
    ...
    function handleNewStaff(e) {
        ...
        if (editingStaffId !== null) {
            // Update mode
            const index = staffRoster.findIndex(s => s.id === editingStaffId);
            if (index !== -1) {
                staffRoster[index] = {
                    ...staffRoster[index],
            ...
            staffRoster.push({
                id: Date.now(),
                name, username, password, pin, role, shift, active: true
            });
