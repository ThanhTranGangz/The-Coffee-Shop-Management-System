...
public class OrderDAO {
    ...
    public void update(Order order) {
        String updateOrderSql = "UPDATE dbo.Orders SET status = ?, updatedAt = ?, notes = ?, totalAmount = ? WHERE id = ?";
        String updateItemsSql = "UPDATE dbo.OrderItems SET status = ? WHERE id = ? AND orderId = ?";
        DBContext db = new DBContext();
        try (Connection con = db.getConnection()) {
            con.setAutoCommit(false);
            try {
                ...
                try (PreparedStatement st = con.prepareStatement(updateOrderSql)) {
                    ...
                    st.setString(2, order.getUpdatedAt());
                    st.setString(3, order.getNotes());
                    st.setInt(4, order.getTotalAmount());
                ...
                if (order.getItems() != null) {
                    for (OrderItem item : order.getItems()) {
                        try (PreparedStatement st = con.prepareStatement(updateItemsSql)) {
                            st.setString(1, item.getStatus());
